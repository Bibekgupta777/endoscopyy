...
/* ═══════════════════════════════════════════════════════════════════
   ✅ HELPER: Resolve performing doctor from patient's doneDoc field
   Priority: patient.doneDoc → match in settings doctors → fallback
   ═══════════════════════════════════════════════════════════════════ */
const resolvePerformingDoctor = (patientDoneDoc, settingsDoctors) => {
  if (!patientDoneDoc) return null; // No doctor on patient, caller decides fallback

  const doctors = settingsDoctors || [];

  // Try exact match (case-insensitive)
  const matched = doctors.find(
    d => d.name.toLowerCase().trim() === patientDoneDoc.toLowerCase().trim()
  );

  if (matched) {
    // Full match: includes name, qualification, signature from settings
    return { ...matched };
  }

  // No match in settings — still use the patient's doctor name
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // ✅ CHANGED: Only set default doctor from settings when NO patient is involved
        ...
        if (preSelectedPatientId && !id) {
          ...
          if (matchedPatient) {
            // ✅ CHANGED: Resolve doctor from patient.doneDoc first
            const performingDoc =
              resolvePerformingDoctor(matchedPatient.doneDoc, settingsDoctors) ||
              settingsDoctors[0] ||
        ...
        // ✅ FIXED: Inside the init() function, replace the existing `if (id)` block with this:
...
if (id) {
  ...
  // ✅ Keep the full patient object reference before overwriting
  const populatedPatient = (typeof fetchedData.patient === 'object' && fetchedData.patient !== null)
    ? fetchedData.patient
    : null;
  ...
  if (populatedPatient) {
    ...
    // ✅ NEW: Backfill empty report fields from patient data
    setFormData(prev => ({
      ...prev,
      ...
      performingDoctor: (prev.performingDoctor?.name)
        ? prev.performingDoctor
        : resolvePerformingDoctor(populatedPatient.doneDoc, settingsRes.data?.doctors) ||
          settingsRes.data?.doctors?.[0] ||
          { name: '', qualification: '', signature: '' }
    }));
  }
  ...
  const resetForNextPatient = () => {
    setFormData(prev => ({
      ...
      procedureDate: new Date().toISOString().split('T')[0],
      procedureTime: new Date().toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' }),
      // ✅ Reset to empty — next patient's doneDoc will fill this
      performingDoctor: { name: '', qualification: '', signature: '' },
      structuredFindings: {
        oralCavity: '',
        oesophagus: { upper: 'Normal', middle: 'Normal', lowerGE: 'At 38 cms' },
  ...
  const getSlotLabel = (index) => {
    const labels = ["Image 1","Image 2","Image 3","Image 4","Image 5","Image 6","Image 7","Image 8","Image 9","Image 10"];
    return labels[index] || `Image ${index + 1}`;
  };
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[100dvh] bg-gray-100 overflow-hidden">
      <div className="flex flex-col w-full lg:w-[60%] h-full bg-white shadow-2xl z-10 border-r">
        <header className="bg-white px-4 py-3 border-b flex justify-between items-center shadow-sm z-20">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-900">{id ? 'Edit Report' : 'Create Report'}</h1>
              <p className="text-xs text-gray-500">
                {activePatientData ? (
                  <span className="text-blue-600 font-medium">{activePatientData.name}</span>
                ) : 'Select a patient'}
          ...
          <div className="flex items-center gap-2">
            <button onClick={handleQuickSave} disabled={saving || !formData.patient}
              className={clsx("px-3 py-2 text-sm font-bold rounded-lg flex items-center gap-1.5 transition-all",
                formData.patient ? "bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-500/20" : "bg-gray-200 text-gray-400 cursor-not-allowed")}>
              <Camera size={16} /><span className="hidden sm:inline">Quick Save</span>
              {totalNewImages > 0 && <span className="bg-white/20 px-1.5 py-0.5 rounded text-xs ml-1" title="New captures">+{totalNewImages}</span>}
            </button>
            <button onClick={handleSubmit} disabled={saving} className="px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-lg shadow-lg shadow-blue-500/20 flex items-center gap-1.5 disabled:opacity-50 transition-all">
              <Save size={16} /><span className="hidden sm:inline">{saving ? 'Saving...' : 'Save & Print'}</span>
            </button>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50/50 pb-32 custom-scrollbar">
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      filteredPatients.map(p => (
                        <button
                          ...
                          onClick={() => {
                            ...
                            // ✅ CHANGED: Always resolve doctor from patient.doneDoc first
                            const resolvedDoc = resolvePerformingDoctor(
                              p.doneDoc,
                              settings?.doctors
                            );

                            if (resolvedDoc) {
                              // Patient has a doneDoc — use it (with settings enrichment if matched)
                              updateField('performingDoctor', resolvedDoc);
                            } else if (settings?.doctors?.length) {
                              // Patient has no doneDoc — fallback to first settings doctor
                              updateField('performingDoctor', settings.doctors[0]);
                            }
                            // else: leave performingDoctor empty
                          }}
                          className="w-full text-left p-3 hover:bg-blue-50 border-b last:border-0"
                        >
                          <div className="font-semibold text-sm">{p.name ?? 'Unnamed'}</div>
                          <div className="text-xs text-gray-500">
                            ...
                            {p.doneDoc && (
                              <span className="ml-2 text-emerald-600 font-medium">• Dr. {p.doneDoc}</span>
                            )}
                          </div>
                        </button>
                      ))
              ...
              <div>
                ...
                <input type="date" value={formData.procedureDate} onChange={(e) => updateField('procedureDate', e.target.value)} className="w-full border-b-2 border-gray-200 bg-gray-50/50 rounded-t p-2 mt-1 focus:border-blue-600 focus:outline-none" />
              </div>

              {/* Time */}
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-1"><Clock size={12} /> Time</label>
                <input type="time" value={formData.procedureTime} onChange={(e) => updateField('procedureTime', e.target.value)} className="w-full border-b-2 border-gray-200 bg-gray-50/50 rounded-t p-2 mt-1 focus:border-blue-600 focus:outline-none" />
              </div>

              {/* Indication */}
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Indication</label>
                <input list="indications" value={formData.indication} onChange={(e) => updateField('indication', e.target.value)} className="w-full border-b-2 border-gray-200 bg-gray-50/50 rounded-t p-2 mt-1 focus:border-blue-600 focus:outline-none" placeholder="e.g., Dyspepsia" />
                <datalist id="indications">{settings?.indications?.map((ind, i) => <option key={i} value={ind} />)}</datalist>
              </div>

              {/* Referring Doctor */}
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Referring Doctor</label>
                <input type="text" value={formData.referringDoctor} onChange={(e) => updateField('referringDoctor', e.target.value)} className="w-full border-b-2 border-gray-200 bg-gray-50/50 rounded-t p-2 mt-1 focus:border-blue-600 focus:outline-none" />
              </div>

              {/* Patient ID */}
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Patient ID</label>
                <input type="text" value={formData.opdIpdNumber} onChange={(e) => updateField('opdIpdNumber', e.target.value)} className="w-full border-b-2 border-gray-200 bg-gray-50/50 rounded-t p-2 mt-1 focus:border-blue-600 focus:outline-none" />
              </div>

              {/* Bill Number */}
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Bill Number</label>
                <input type="text" value={formData.billNumber} onChange={(e) => updateField('billNumber', e.target.value)} className="w-full border-b-2 border-gray-200 bg-gray-50/50 rounded-t p-2 mt-1 focus:border-blue-600 focus:outline-none" />
              </div>

              {/* ✅ CHANGED: Examined By — Shows source (patient vs settings) */}
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Examined By *</label>
                <select
                  className="w-full border-b-2 border-gray-200 bg-gray-50/50 rounded-t p-2 mt-1 focus:border-blue-600 focus:outline-none"
                ...
                >
                  ...
                  {/* Show patient's doctor if not in settings list */}
                  {formData.performingDoctor?.name &&
                    !settings?.doctors?.some(d => d.name === formData.performingDoctor?.name) && (
                      <option value={-1}>{formData.performingDoctor.name}</option>
                    )}
                ...
                {activePatientData?.doneDoc && formData.performingDoctor?.name === activePatientData.doneDoc && (
                  <p className="text-[10px] text-emerald-600 mt-1 font-medium">
                    ✓ Auto-filled from patient record (Dr. {activePatientData.doneDoc})
                  </p>
                )}
                {activePatientData && !activePatientData.doneDoc && formData.performingDoctor?.name && (
                  <p className="text-[10px] text-amber-600 mt-1 font-medium">
                    ⚠ Patient has no assigned doctor — using default from settings
                  </p>
                )}
              </div>
              ...
              {/* Consent */}
              <div className="flex items-center gap-3 pt-4">
                <input type="checkbox" id="consent" checked={formData.consentObtained} onChange={(e) => updateField('consentObtained', e.target.checked)} className="w-5 h-5 rounded border-gray-300 text-blue-600 cursor-pointer" />
                <label htmlFor="consent" className="font-semibold text-gray-700 cursor-pointer text-sm">Informed Consent Obtained</label>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <h3 className="font-bold text-gray-700 bg-gray-100 p-1.5 px-3 rounded text-sm">Oesophagus</h3>
                {['upper', 'middle', 'lowerGE'].map(k => (
                  <div key={k} className="grid grid-cols-[80px_1fr] items-center gap-2">
                    <span className="text-sm text-gray-600 text-right">{k === 'lowerGE' ? 'Lower G-E:' : k.charAt(0).toUpperCase() + k.slice(1) + ':'}</span>
                    <input type="text" value={formData.structuredFindings.oesophagus[k]} onChange={(e) => updateField(`structuredFindings.oesophagus.${k}`, e.target.value)} className="border-b focus:border-blue-600 outline-none p-1 text-sm" />
                  </div>
                ))}
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-gray-700 bg-gray-100 p-1.5 px-3 rounded text-sm">Stomach</h3>
                {['fundus', 'body', 'antrum', 'pRing'].map(k => (
                  <div key={k} className="grid grid-cols-[80px_1fr] items-center gap-2">
                    <span className="text-sm text-gray-600 text-right">{k === 'pRing' ? 'P-Ring:' : k.charAt(0).toUpperCase() + k.slice(1) + ':'}</span>
                    <input type="text" value={formData.structuredFindings.stomach[k]} onChange={(e) => updateField(`structuredFindings.stomach.${k}`, e.target.value)} className="border-b focus:border-blue-600 outline-none p-1 text-sm" />
                  </div>
                ))}
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-gray-700 bg-gray-100 p-1.5 px-3 rounded text-sm">Duodenum</h3>
                {['bulb', 'd2', 'papilla'].map(k => (
                  <div key={k} className="grid grid-cols-[80px_1fr] items-center gap-2">
                    <span className="text-sm text-gray-600 text-right">{k === 'd2' ? 'D2:' : k.charAt(0).toUpperCase() + k.slice(1) + ':'}</span>
                    <input type="text" value={formData.structuredFindings.duodenum[k]} onChange={(e) => updateField(`structuredFindings.duodenum.${k}`, e.target.value)} className="border-b focus:border-blue-600 outline-none p-1 text-sm" />
                  </div>
                ))}
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-gray-700 bg-gray-100 p-1.5 px-3 rounded text-sm">Comments</h3>
                <textarea rows="4" className="w-full border border-gray-200 rounded p-2 text-sm focus:border-blue-600 focus:outline-none bg-gray-50/50" value={formData.structuredFindings.comments} onChange={(e) => updateField('structuredFindings.comments', e.target.value)} placeholder="Additional findings, impressions..." />
              </div>
            </div>
          </div>
              ...
              {reportSlots.map((slot, index) => (
                <div key={index} className="flex flex-col">
                  <div
                    ...
                    draggable={slot !== null}
                    onDragStart={(e) => slot && handleDragStart(e, 'slot', index)}
                    className={clsx("relative aspect-[4/3] rounded-lg border-2 transition-all flex flex-col items-center justify-center overflow-hidden group",
                      slot === null ? "border-dashed border-gray-300 bg-gray-50" : "border-solid border-blue-400 bg-black cursor-grab active:cursor-grabbing shadow-md hover:shadow-lg")}
                  >
                    {slot === null ? (
                      <div className="text-gray-400 flex flex-col items-center p-2 text-center pointer-events-none">
                        <ImagePlus size={18} className="mb-1 opacity-50" />
                      ...
                      <>
                        <button type="button" onClick={() => removeSlotImage(index)} className="absolute top-1 right-1 bg-red-500 text-white rounded-full p-1 z-10 shadow hover:bg-red-600 opacity-0 group-hover:opacity-100 transition-opacity"><X size={10} /></button>
                        <div className="absolute top-1 left-1 bg-black/50 text-white rounded-full p-1 z-10 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"><GripHorizontal size={10} /></div>
                  ...
                  <input
                    type="text" placeholder="Caption..." value={slot ? slot.caption || '' : ''}
                    onChange={(e) => { if (!slot) return; const newSlots = [...reportSlots]; newSlots[index] = { ...newSlots[index], caption: e.target.value }; setReportSlots(newSlots); }}
                    disabled={slot === null}
                    className={clsx("mt-1 w-full text-[10px] p-1 border rounded text-center font-medium outline-none transition-colors",
                      slot === null ? "bg-gray-100 border-gray-200 text-gray-400" : "bg-white border-blue-300 focus:border-blue-600 text-gray-800")}
                  />
                </div>
              ))}
            </div>
      ...
      <div className="flex flex-col w-full lg:w-[40%] h-[50vh] lg:h-full bg-gray-900 shadow-2xl relative">
        <div className="bg-gray-800 p-3 flex justify-between items-center z-10 border-b border-gray-700">
          <h2 className="text-white font-bold text-sm flex items-center gap-2"><Camera size={16} /> Live Capture</h2>
          <div className="flex bg-gray-900 rounded-lg p-1 border border-gray-700">
            <button type="button" onClick={() => setCameraSource('pentax')} className={clsx('px-3 py-1 text-xs font-bold rounded-md transition-colors', cameraSource === 'pentax' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white')}>Endo</button>
            <button type="button" onClick={() => setCameraSource('webcam')} className={clsx('px-3 py-1 text-xs font-bold rounded-md transition-colors', cameraSource === 'webcam' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white')}>Webcam</button>
          </div>
        </div>

        <div className="flex-shrink-0 bg-black aspect-[4/3] w-full flex items-center justify-center overflow-hidden border-b-2 border-blue-900">
          {cameraSource === 'pentax' ? (
            <PentaxLiveFeed onCapture={async (file) => { const c = await compressImage(file); setCapturedPool(prev => [{ file: c, preview: URL.createObjectURL(c), caption: '' }, ...prev]); }} />
          ) : (
            <EndoCamera patientInfo={activePatientData} onCapture={async (file) => { const c = await compressImage(file); setCapturedPool(prev => [{ file: c, preview: URL.createObjectURL(c), caption: '' }, ...prev]); }} />
          )}
        </div>

        <div className={clsx("flex-1 overflow-y-auto p-4 custom-scrollbar transition-all bg-gray-800", isDraggingOverPool ? "border-t-2 border-blue-500 bg-gray-800/80" : "")}
          onDragOver={(e) => { e.preventDefault(); setIsDraggingOverPool(true); }} onDragLeave={() => setIsDraggingOverPool(false)} onDrop={handleDropOnPool}>
          <div className="flex justify-between items-center mb-3">
            <h3 className="text-gray-300 font-bold text-xs uppercase tracking-widest flex items-center gap-2">
              Captured Pool ({capturedPool.length})
              {isDraggingOverPool && <span className="text-[10px] text-blue-400 animate-pulse">Drop here to remove from slot</span>}
            ...
            {capturedPool.length > 0 && (
              <button type="button" onClick={() => setCapturedPool([])} className="text-red-400 text-xs hover:text-red-300 font-medium bg-red-400/10 px-2 py-1 rounded">Clear All</button>
            )}
          </div>
          {capturedPool.length === 0 ? (
            <div className="h-32 flex flex-col items-center justify-center text-gray-500 border-2 border-dashed border-gray-600 rounded-xl p-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              {capturedPool.map((img, idx) => (
                <div key={idx} draggable onDragStart={(e) => handleDragStart(e, 'pool', idx)} className="relative aspect-[4/3] border-2 border-gray-600 rounded bg-black cursor-grab active:cursor-grabbing hover:border-blue-500 transition-colors overflow-hidden group">
                  <img src={img.preview || getImageURL(img.existingPath)} className="w-full h-full object-cover pointer-events-none opacity-80 group-hover:opacity-100 transition-opacity" alt={`Capture ${idx}`} />
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"><div className="bg-black/70 text-white text-[10px] font-bold px-2 py-1 rounded">DRAG TO SLOT</div></div>
                  <button type="button" onClick={() => removePoolImage(idx)} className="absolute top-1 right-1 bg-red-600 text-white rounded p-0.5 z-10 hover:bg-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><X size={12} /></button>
                </div>
              ))}
            </div>
          )}
